...
while (<DATA>) {
    chomp;
    next if /^\#/;
    my ($no, $description, $sort_code, $account_number, $expected) = split /\|/;
    my $account = BankAccount::Validator::UK->new();
    my $actual = $account->is_valid($sort_code, $account_number) ? 'Y' : 'N';
    ok($actual eq $expected, sprintf("Test %d FAILED: %s", $no, $description));
}
...
19|Exception 2 & 9 where the first check passes and second check fails.|309070|02355688|Y
...
31|Exception 12/13 where passes the modulus 11 check (in this example, modulus 10 check fails, however, there is no need for it to be performed as the first check passed).|074456|12345112|Y
32|Exception 12/13 where passes the modulus 11 check (in this example, modulus 10 check passes as well, however, there is no need for it to be performed as the first check passed).|070116|34012583|Y
